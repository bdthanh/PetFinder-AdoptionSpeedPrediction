...
class GaussianNBTrainer:
    ...
    def optimize_hyperparameters(self):
        logging.info("Start optimizing hyperparameters for GaussianNB 200 trials")
        study = optuna.create_study(direction="maximize")
        study.optimize(self.objective, n_trials=1)
        self.best_params = study.best_params
        logging.info(f"Best params for GaussianNB: {self.best_params}")
        return study.best_params
    
    def train_model(self):
        logging.info("Training GaussianNB model with best params")
        self.optimize_hyperparameters()
        with open(os.path.join(self.model_dir, 'gaussian_nb_best_params.json'), 'w') as f:
            json.dump(self.best_params, f)
        final_model = GaussianNB(**self.best_params)
        final_model.fit(self.X_train, self.y_train)
        logging.info("Saving GaussianNB model")
    ...
    def evaluate_model(self):
        ...
        plt.savefig(os.path.join(self.model_dir, 'gaussian_nb_confusion_matrix.png'))
      
if __name__ == "__main__":
    trainer = GaussianNBTrainer()
    trainer.train_model()
