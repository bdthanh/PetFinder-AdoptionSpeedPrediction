...
class KNNTrainer:
    def __init__(self, type: str) -> None:
        ...
        self.model_path = os.path.join(self.model_dir, "knn_model.pkl")
    ...
    def optimize_hyperparameters(self):
        logging.info("Start optimizing hyperparameters for KNN 100 trials")
        study = optuna.create_study(direction="maximize")
        study.optimize(self.objective, n_trials=1)
        self.best_params = study.best_params
        logging.info(f"Best params for KNN: {self.best_params}")
    
    def train_model(self):
        logging.info("Training KNN model with best params")
        self.optimize_hyperparameters()
        with open(os.path.join(self.model_dir, 'knn_best_params.json'), 'w') as f:
    ...
    def evaluate_model(self):
        ...
        plt.savefig(os.path.join(self.model_dir, 'knn_confusion_matrix.png'))
