...
from U_utils import save_obj, load_obj, get_filepath, get_evaluation_info, has_tuned_params, has_models, create_if_missing_folder
from U_constants import NOT_ORDINAL_CAT_COLS, MODEL_DIR, NUM_FOLD, NUM_TRIALS, TUNED_PARAMS_PATH, RANDOM_STATE, DATA_DIR
...
class LightGBMTrainer:
    ...
    def objective(self, trial):
        ...
        for fold in range(NUM_FOLD):
            X_train, y_train, X_test, y_test, X_valid, y_valid = self.get_data(fold)
    ...
    def train_model(self):
        ...
        for fold in range(NUM_FOLD):
            ...
            save_obj(os.path.join(MODEL_DIR, f"lightgbm_model_{fold}.pkl"), model)
    ...
    def evaluate_model(self):
        ...
        for fold in range(NUM_FOLD):
            X_train, y_train, X_test, y_test, X_valid, y_valid = self.get_data(fold)
            model = load_obj(os.path.join(MODEL_DIR, f"lightgbm_model_{fold}.pkl"))
